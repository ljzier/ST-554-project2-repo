...
class SparkDataCheck:
    ...
    def check_col_range(self, column, lower=None, upper=None):
        ...
        #do the comparison
        if lower is None:
            self.df= self.df.withColumn('num_check', F.col(column) <= upper)
        elif upper is None:
            self.df= self.df.withColumn('num_check', F.col(column) >= lower)
        else:
            self.df= self.df.withColumn('num_check', F.col(column).between(lower, upper))  
        return self
        ...
        if column is not None:
            ...
            if (group is None):
              ...
              # column and group present
                result = (self.df.groupBy(group).agg(F.min(F.col(f'`{column}`')),
                          F.max(F.col(f'`{c}`')))).toPandas()
                result.insert(0, 'col_name', column)
        ...
        # no column supplied so min/max for all numeric
            ...
            # if group but no column
                ...
                for c in num_cols:
                    ...
                    dfc = self.df.groupBy(group).agg(F.min(F.col(f'`{c}`')), F.max(F.col(f'`{c}`'))).toPandas()                  
                    dfc = dfc.rename(columns={f'min({c})': 'min', f'max({c})':'max'})
                    dfc['col_name']=c
                    dfs.append(dfc)
                                            
                # combine into one dataframe
                result=reduce(lambda a,b:pd.merge(a,b, on=group),dfs)
                return result
